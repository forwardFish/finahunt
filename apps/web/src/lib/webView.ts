...
const BROKEN_FRAGMENTS = ["\u951f", "\ufffd", "\u9225", "\u95bf", "\u942e", "\u9359", "\u68f0", "\u6d63", "\u93c3", "\u8930", "\u7a0c", "\u93b6", "\u93b5", "\u93c8", "\u6d93", "\u695d", "\u7f01", "\u5bf0"];
...
export function sourceLabel(sourceIdOrName: string | null | undefined): string {
  ...
  const labels: Record<string, string> = {
    "cls-telegraph": "财联社电报",
    "jiuyangongshe-live": "韭研公社",
    "xueqiu-hot-spot": "雪球热点",
  };
...
export function summarizeResearchState(state: "success" | "partial" | "empty"): { tone: "ready" | "partial" | "empty"; label: string; description: string } {
  ...
  return { tone: "empty", label: "暂无研究结果", description: "当前日期还没有低位研究工作台数据，可执行低位挖掘或切换到已有结果日期。" };
}
